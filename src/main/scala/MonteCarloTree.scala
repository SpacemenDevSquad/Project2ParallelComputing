...
class MonteCarloTree {
  def MonteCarloTree[A, B](decision: (Double, A) => A, isComplete: A => Boolean, winner: (A, B) => Boolean, currMove: A, player: B, isParallel: Boolean = true, iterations: Int = 20000000): A = {

    def SeqTree(): A = {
      val decisionMap = mutable.Map[A, Double]()
      for i <- 0 until iterations do {
        val tryMove: A = decision(Random().nextDouble(), currMove)
    ...
    @tailrec
    def RunScenario(currMove: A): Boolean = if isComplete(currMove) then winner(currMove, player) else RunScenario(decision(Random().nextDouble(), currMove))
